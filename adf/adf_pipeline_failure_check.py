...
TEAMS_WEBHOOK_URL = (
    "https://prod-40.usgovtexas.logic.azure.us:443/workflows/"
    "395b977041484d84a8f929bed18f5649/triggers/manual/paths/invoke"
    "?api-version=2016-06-01&sp=%2Ftriggers%2Fmanual%2Frun&sv=1.0"
    "&sig=HHt_jJbBXaTc7Ta2k9plYggLa7zgh1Vt6YzlYDwj26c"
)
...
LOG_FILE = "/home/thedavidporter/adf_pipeline_failures.log"
...
def main():
    parser = argparse.ArgumentParser(description="ADF pipeline failure checker")
    parser.add_argument(
        "--no-notify",
        action="store_true",
        help="Run all checks but skip Teams and Pushover notifications.",
    )
    ...
    if all_failures and not args.no_notify:
        send_teams(all_failures, lookback_hours, generated_at)
        send_pushover(all_failures, lookback_hours)
    elif all_failures and args.no_notify:
        log.info("--no-notify set: skipping Teams and Pushover notifications.")

    if query_errors == len(ENVIRONMENTS):
        log.error("All environment queries failed — check credentials/connectivity.")
        sys.exit(1)
