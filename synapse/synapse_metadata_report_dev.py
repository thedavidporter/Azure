...
.modal-ai-desc{font-size:12px;color:var(--txt);line-height:1.6;
  padding:8px 16px 10px;border-bottom:1px solid var(--brd);
  background:var(--sur2);border-left:3px solid var(--acc)}
...
// COL_DATA format: [schema, table, ordinal, col_name, type, len, nullable, default, ai_desc]
...
function openDetail(key, el){
  ...
  const descEl = document.getElementById('modal-ai-desc');
  if(meta.desc){
  ...
  if(cols.length){
    ...
    const rows = cols.map(c => {
      ...
      const descCell = hasDesc
        ? '<td class="col-def" style="color:var(--mut)">' + escH(c.desc||'') + '</td>'
        : '';
      return '<tr>'
    ...
    const descHeader = hasDesc ? '<th>Description</th>' : '';
    colBody.innerHTML =
      '<table class="modal-col-tbl">'
      + '<thead><tr><th class="col-ord">#</th><th>Column Name</th>'
      + '<th>Data Type</th><th>Nullable</th><th>Default</th>' + descHeader + '</tr></thead>'
...
def build_html(schemas, objects, columns, fks, deps, defs, row_counts, generated):
    ...
    # ── plain-English descriptions (AI-generated via generate_descriptions.py) ──
    import os as _os
    ...
    # Format per entry: [schema, table, ordinal, col_name, type, len, nullable, default]
    col_data = [
...
def main():
    ...
    snap_file = f"{snap_dir}/synapse_dev_{datetime.now().strftime('%Y%m%d')}.json"
    snapshot = {
