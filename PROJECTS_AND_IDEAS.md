...
| 2026-07-21 | Security Groups report — Excel export button added to group detail dialog; clicking "⬇ Excel" downloads a SpreadsheetML `.xls` file with two sheets: Members (Name, Type, Email/UPN) and Role Assignments (Resource Type, Resource, Role, Resource Group, Subscription); pure client-side, no library required; filename is the group display name |
