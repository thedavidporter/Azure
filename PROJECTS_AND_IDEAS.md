...
| Date | Item |
|------|------|
| 2026-07-23 | Help page — sidebar nav links now jump instantly to section anchors (no smooth scroll); changed `scrollIntoView({behavior:'smooth'})` to `behavior:'instant'`; also added AKS glossary entry (Azure Kubernetes Service — JupyterHub context, D-series/B-series VM cost tie-in); deployed |
| 2026-07-23 | Azure Cost Report — VM Purpose labels corrected: queried `az vm list` and `az vmss list` on Shared sub and discovered all D-series and B2ms costs come from AKS node pools (not SHIR); D4s_v3 = `aks-system-*` (8 nodes DEV+PRD), D8_v3 = `aks-jhub-*` (JupyterHub, 2 nodes), D4s_v5 = `aks-agentpool/userpool` (DEV v3), D8_v5 = `aks-jhub-*` (DEV v3), B2ms = `aks-default-*`; updated `VM_PURPOSE_MAP` and `_vm_purpose()` to match on combined meter+tier string so D4 vs D8 within same Dv3 tier are distinguished; deployed |
| 2026-07-22 | Azure Cost Report — VM Meter Breakdown pop-up: added Purpose column mapping each VM size to its role (AVD Session Host, SHIR / DevOps Agent, Dev / Utility); driven by `VM_PURPOSE_MAP` constant at top of `azure_cost_report.py` for easy updates; deployed |
| 2026-07-22 | Azure Cost Report — savings opportunity rows now have clickable ⓘ Details buttons; each opens a modal with meter-level breakdown (title, explanation, table, note); detail keys vm/db/fw/la; data fetched via 2 broad `fetch_all_meters` calls (one per sub) then filtered in Python to avoid 429 rate limits; added to `publish_synapse_metadata.sh` so cost report regenerates on every publish run; deployed |
