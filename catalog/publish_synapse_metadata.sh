...
run_step() {
  ...
  # Update progress banner after each report step (skip for the index step itself)
  if [[ "$blob_name" != "index.html" ]]; then
    advance_progress
  fi
}

# Re-generate index with current progress and re-upload the banner
advance_progress() {
  PUBLISH_STEP=$((PUBLISH_STEP + 1))
  $VENV /home/thedavidporter/generate_metadata_index.py \
    --running "$START_TS" --step "$PUBLISH_STEP" --total "$PUBLISH_TOTAL" 2>/dev/null \
    && upload_blob "index.html" "/home/thedavidporter/index.html" 2>/dev/null || true
}

# Upload a "refresh in progress" index immediately so visitors see a banner
START_TS=$(date '+%Y-%m-%d %H:%M')
echo ""
echo "[0/47] Uploading in-progress index banner..."
if $VENV /home/thedavidporter/generate_metadata_index.py --running "$START_TS"; then
  upload_blob "index.html" "/home/thedavidporter/index.html" \
    || echo "[0/47] WARNING: in-progress banner upload failed — continuing"
else
  echo "[0/47] WARNING: in-progress index generation failed — continuing"
fi
...
run_step "39/47" "40/47" "Data Catalog (REDCap data sharing requests)" \
...
run_step "41/47" "42/47" "Azure Security Groups & Access report" \
  "azure_security_groups_report.html" "/home/thedavidporter/azure_security_groups_report.html" \
  $VENV /home/thedavidporter/azure_security_groups_report.py

# ── DATABRICKS (last — slow PRD notebook walk; timeout 900s prevents indefinite hang) ──
run_step "43/47" "44/47" "Databricks metadata report (all 3 workspaces)" \
...
run_step "47/47" "47/47" "metadata index page" \
...
if [ -f "$DBKS_TOKEN_FILE" ] && [ -x "$DBKS_CLI" ]; then
  ...
  if (cd "$DBKS_PROJECT" && DATABRICKS_HOST="$DBKS_HOST" DATABRICKS_TOKEN="$DBKS_TOKEN" \
    ...
    echo "  Databricks bundle deploy complete — redeploying app..."
    DATABRICKS_HOST="$DBKS_HOST" DATABRICKS_TOKEN="$DBKS_TOKEN" \
      ...
      2>&1 && echo "  App redeployed" || echo "  [dbks] WARN: app redeploy failed"
    echo "  App: https://idoh-metadata-marketplace-5757046586469840.0.azure.databricksapps.com"
  else
    echo "  [dbks] WARN: bundle deploy failed — reports may be stale"
  fi
else
  echo ""
  echo "[dbks] Skipping Databricks deploy — $DBKS_TOKEN_FILE not found or CLI missing"
...
echo " https://zus1idohdevv2dbrkdl.z13.web.core.windows.net/azure_security_groups_report.html"
