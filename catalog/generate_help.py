...
<p>Branch policies (required reviewers, build validation) are also shown so you can verify governance rules are in place.</p>""",
    ...
    q_and_a += qa(
        ...
        """<p>Every report shows a color-coded <strong>↻ YYYY-MM-DD HH:MM EDT</strong> timestamp — both on each index page card and at the top of the individual report itself.</p>
<ul>
  <li>A <span style="color:var(--grn)">green</span> timestamp means the report is current (refreshed within the last 25 hours).</li>
  <li>A <span style="color:var(--yel)">yellow</span> timestamp means the report is stale — not refreshed in over 25 hours but within the last week.</li>
  <li>A <span style="color:var(--red)">red</span> timestamp means the report has not been refreshed in over a week (or the file does not yet exist).</li>
</ul>
<p>All timestamps are in <strong>Eastern Time (EST/EDT)</strong>. Reports are typically regenerated by the nightly <code>publish_synapse_metadata.sh</code> run.</p>""",
        "all"
    )
...
<p class="tip">Containers that are growing rapidly but not being read by any active pipeline may be accumulating data without a retention policy. Cross-reference with the ADF report to check whether those paths are referenced by any active datasets.</p>""",
    ...
    q_and_a += qa(
        "How do I export security group members to Excel?",
        """<p>Open the <strong>Security Groups &amp; Access Report</strong> and click any group card to open its detail dialog.</p>
<ol>
  <li>Click the green <strong>⬇ Excel</strong> button in the top-right of the dialog (next to the close button).</li>
  <li>The download is a true <code>.xlsx</code> file with two sheets: <em>Members</em> (Name, Type, Email/UPN) and <em>Role Assignments</em> (Resource Type, Resource, Role, Resource Group, Subscription).</li>
  <li>For <strong>Databricks Direct</strong> workspace cards, the Excel export includes a Members sheet listing only the direct-access users (those added to the workspace without an Azure group).</li>
</ol>
<p class="tip">The file is generated entirely in the browser — no data leaves your session. SheetJS loads on the first export click; subsequent exports are instant.</p>""",
        "engineer"
    )

    q_and_a += qa(
        "How do I open a dataset's schema directly in the Synapse Metadata Report?",
        """<p>In the <strong>IDOH Data Catalog</strong>, click any dataset card to open its detail modal.</p>
<ol>
  <li>Scroll to the <strong>Synapse Schemas</strong> section — each schema is shown as a pill (e.g. <code>SM_CHIRP_01</code>).</li>
  <li>Each pill has a <strong>PRD↗</strong> link and a <strong>DEV↗</strong> link. Clicking either opens the Synapse Metadata Report in a new tab, pre-filtered to that schema's tables and views.</li>
  <li>The report opens directly on the <em>All Objects</em> tab with the schema filter already applied — no need to manually search or navigate after opening.</li>
</ol>""",
        "analyst"
    )
...
<div class="main">
  ...
  <h1>Help &amp; Guide</h1>
  <p class="meta">IDOH Azure Metadata Marketplace &nbsp;&middot;&nbsp; Generated: <span id="gen-ts" data-ts="{generated}">&#x21BB; {generated}</span><script>(function(){{var s=document.getElementById('gen-ts'),h=(Date.now()-new Date(s.dataset.ts.replace(/ [A-Z]{{2,4}}$/,'').replace(' ','T')))/36e5;s.style.color=h<25?'var(--grn)':h<168?'var(--yel)':'var(--red)';s.style.fontWeight='700';}})();</script></p>
  ...
  <div class="report-grid">
    ...
    <div class="report-card">
      ...
      <p>All Entra ID security groups with their Azure role assignments across every subscription — group members, roles held, and which Synapse, Databricks, ADF, Key Vault, and storage resources each group can access. Click any group to open its detail dialog; use the <strong>⬇ Excel</strong> button to download the member list and role assignments as a <code>.xlsx</code> file. Also covers Databricks Direct users — accounts added directly to a workspace without an Azure group. Use this to audit who has access to what and confirm least-privilege assignments.</p>
      <div class="tags">
        <span class="tag tag-all">ALL ENVS</span>
  ...
  <div id="gl-list">
    ...
    <div class="gl-row"><div class="gl-term">AKS (Azure Kubernetes Service)</div><div class="gl-def">Microsoft's managed Kubernetes platform for running containerized applications across a cluster of VMs. In the Shared subscription, AKS hosts JupyterHub — a multi-user notebook server used by data scientists and analysts. The D-series and B-series VM costs in the Azure Cost Report come from AKS node pools, not standalone VMs.</div></div>
    ...
    <div class="gl-row"><div class="gl-term">Commitment Tier</div><div class="gl-def">A pre-committed daily data ingestion level for Azure Log Analytics (e.g., 100 GB/day). Priced lower per GB than Pay-As-You-Go — roughly 30% savings for predictable workloads. Appears as a savings opportunity in the Executive Cost Report when Log Analytics spend exceeds the pay-as-you-go threshold.</div></div>
    ...
    <div class="gl-row"><div class="gl-term">Databricks Notebook</div><div class="gl-def">The primary interactive development environment in Databricks. Combines code cells (Python, SQL, Scala, R) with markdown text and visual output in a single document. Notebooks stored outside /Repos are not version-controlled unless manually exported.</div></div>
    ...
    <div class="gl-row"><div class="gl-term">DBU (Databricks Unit)</div><div class="gl-def">The billing unit for Databricks compute. One DBU represents one unit of processing capacity per hour. Billed at different rates depending on compute type (All-Purpose, Jobs, SQL, DLT) and tier (Standard, Premium). Visible as a meter line in the Databricks cost breakdown in the Executive Cost Report.</div></div>
    ...
    <div class="gl-row"><div class="gl-term">Deployment Environment (ADO)</div><div class="gl-def">A named target environment in Azure Pipelines (e.g., Dev, Staging, Production) with deployment history, approval gates, and resource tracking. Used to control what is deployed where and to maintain an audit trail of releases.</div></div>
    ...
    <div class="gl-row"><div class="gl-term">DLT (Delta Live Tables)</div><div class="gl-def">A Databricks framework that declares data transformation logic as a graph of tables. Manages execution order, incremental updates, data quality checks (expectations), and retries automatically. A higher-level alternative to hand-written Spark ETL jobs. Visible as its own tab in the Databricks Metadata Report.</div></div>
    ...
    <div class="gl-row"><div class="gl-term">Entra ID</div><div class="gl-def">Microsoft's cloud identity platform, formerly known as Azure Active Directory (AAD). Manages user accounts, groups, service principals, and managed identities across Azure and Microsoft 365. The Security Groups &amp; Access report enumerates all Entra ID groups that hold role assignments on IDOH Azure resources.</div></div>
    ...
    <div class="gl-row"><div class="gl-term">Host Pool (AVD)</div><div class="gl-def">A collection of Azure Virtual Desktop session host VMs sharing the same configuration and image. Users are assigned to host pools and connect to any available session host within the pool. IDOH Shared Production has 142 host pools covered by the AVD Session Host Inventory report.</div></div>
    ...
    <div class="gl-row"><div class="gl-term">Key Vault Certificate</div><div class="gl-def">A TLS/SSL certificate managed by Azure Key Vault. Key Vault can auto-renew certificates from DigiCert or Let's Encrypt before expiry, eliminating manual PFX exports. Services such as Application Gateway and API Management can pull certificates directly from the vault. Covered by the Key Vault Metadata Report.</div></div>
    ...
    <div class="gl-row"><div class="gl-term">Managed Identity</div><div class="gl-def">An Entra ID identity automatically managed by Azure for a service (e.g. ADF, Synapse, Databricks). Used to authenticate to other Azure services without storing credentials in code or config files. Preferred over service principals because there is no credential to rotate.</div></div>
    ...
    <div class="gl-row"><div class="gl-term">MTD (Month-to-Date)</div><div class="gl-def">Spend accumulated from the first day of the current billing month through today. The headline metric on the Executive Cost Report — reflects actual billed consumption for the period in progress, before the invoice closes at month end.</div></div>
    ...
    <div class="gl-row"><div class="gl-term">Photon (Databricks)</div><div class="gl-def">Databricks' native vectorized query engine, written in C++. Accelerates SQL and DataFrame workloads by processing data in columnar batches. Billed at a higher DBU rate than standard Spark and appears as a separate meter line in the Databricks cost breakdown in the Executive Cost Report.</div></div>
    ...
    <div class="gl-row"><div class="gl-term">RBAC (Role-Based Access Control)</div><div class="gl-def">Azure's permission model that grants access by assigning roles (Owner, Contributor, Reader, or custom) to identities (users, groups, service principals, managed identities) at a defined scope (subscription, resource group, or individual resource). The Security Groups report shows every group's RBAC role assignments across IDOH Azure resources.</div></div>
    ...
    <div class="gl-row"><div class="gl-term">Repos (Databricks)</div><div class="gl-def">Git-integrated folders in a Databricks workspace that sync to a remote repository (Azure DevOps, GitHub). Code in /Repos is version-controlled; edits are committed and pushed through the Databricks UI or CLI. Visible as its own tab in the Databricks Metadata Report.</div></div>
    <div class="gl-row"><div class="gl-term">Reserved Instance (RI)</div><div class="gl-def">A 1- or 3-year compute commitment for Azure VMs or other services that reduces the hourly rate by up to 35–65% vs. pay-as-you-go. No changes to the VM or workload — only the billing rate changes. Requires upfront or monthly payment and coordination with Finance. Appears as the top savings opportunity in the Executive Cost Report for AVD session hosts.</div></div>
    ...
    <div class="gl-row"><div class="gl-term">Service Endpoint (VNet)</div><div class="gl-def">Extends a VNet subnet's identity to Azure services (e.g., Storage, SQL, Key Vault) over the Azure backbone network, keeping traffic off the public internet. Simpler than a private endpoint but less granular — applies at the service type level rather than the individual resource level. Visible in the VNet report's subnet table.</div></div>
    <div class="gl-row"><div class="gl-term">Service Principal</div><div class="gl-def">An application identity in Entra ID used by services, scripts, and automation to authenticate to Azure resources without a user account. Common in ADF linked services, Databricks connections, and CI/CD pipelines. Requires periodic credential rotation unless replaced by a managed identity.</div></div>
    <div class="gl-row"><div class="gl-term">Session Host (AVD)</div><div class="gl-def">A Windows VM that users connect to as their virtual desktop through Azure Virtual Desktop. Session hosts are pooled within a host pool and can serve single or multiple simultaneous user sessions. They appear as FSv2 Windows VMs in the Azure Cost Report and are inventoried in the AVD Session Host Inventory report.</div></div>
    ...
    <div class="gl-row"><div class="gl-term">UPN (User Principal Name)</div><div class="gl-def">The email-format username used to identify a user in Entra ID (e.g., jsmith@health.in.gov). UPNs are used in Azure group memberships, Databricks access provisioning, and audit logs. Exported in the Security Groups &amp; Access report Excel downloads.</div></div>
    <div class="gl-row"><div class="gl-term">Variable Group (ADO)</div><div class="gl-def">A shared collection of variables and secrets in Azure DevOps that can be referenced across multiple pipelines. Variable groups can be linked to Azure Key Vault to pull secrets at pipeline runtime without hard-coding credentials in pipeline definitions.</div></div>
...
async function fbSubmit(){{
  ...
  try {{
    const payload = JSON.stringify({{name, dt, priority: fbPri, comment, page: 'help'}});
    let r;
    try {{
