...
def get_extended(conn, db, tbl):
    ...
    try:
        ...
        for r in rows:
            ...
            if k == 'provider':           info['format']    = v
                ...
                # "Statistics: 3009099 bytes, 0 rows" — stale stats common when ANALYZE not run
        ...
        pass
    # Size is 0/missing → try DESCRIBE DETAIL (Delta) for real file size
    if info['size_bytes'] in ('', '0'):
        try:
            ...
            pass
    # Row count is 0/missing but data exists → stats are stale; do a live count
...
def get_notebooks():
    """Walk Databricks workspace via REST API; return notebooks with language + owner."""
    headers  = {"Authorization": f"Bearer {TOKEN}"}
    base_url = f"https://{HOSTNAME}/api/2.0/workspace"
    notebooks = []
    stack     = ["/"]
    visited   = set()
    ...
    while stack:
        ...
        try:
            ...
            print(f"  WARN workspace/list {path}: {e}")
            continue
        ...
        for obj in resp.json().get("objects", []):
            ...
            if obj_type == "DIRECTORY":
                ...
                if not creator or not language:
                    try:
                        s = requests.get(f"{base_url}/get-status", headers=headers,
                                         params={"path": obj_path}, timeout=30)
                        if s.ok:
                            status = s.json()
                            if not creator:
                        ...
                        pass
                modified_ms = obj.get("modified_at", 0)
                modified_str = (
                    datetime.fromtimestamp(int(modified_ms) / 1000, tz=timezone.utc)
...
/* notebook language badges */
.lang{font-size:10px;padding:2px 7px;border-radius:3px;font-weight:700;white-space:nowrap}
.lang-PYTHON{background:#3a2200;color:#ff9020}
.lang-SQL{background:#001e3a;color:#60a5fa}
.lang-SCALA{background:#3a001e;color:#f87171}
.lang-R{background:#1a3a1a;color:#4ade80}
.lang-unknown{background:#1e1e1e;color:#6b7898}
"""
...
function filterByType(type){
  ...
  // show/hide rows based on chip text
...
def build_html(databases, objects, col_data, notebooks, generated):
    # compact column data for JS
    ...
    # ── notebook rows ─────────────────────────────────────────────────────────
    lang_label = {'PYTHON': 'Python', 'SQL': 'SQL', 'SCALA': 'Scala', 'R': 'R'}
    nb_rows = []
    for nb in sorted(notebooks, key=lambda x: x['path']):
        lang     = nb.get('language', '') or ''
        lang_cls = f'lang-{lang}' if lang else 'lang-unknown'
        lang_lbl = lang_label.get(lang, lang) if lang else '—'
        nb_rows.append(
            f'<tr>'
            f'<td style="max-width:420px;white-space:normal">{esc(nb["path"])}</td>'
            f'<td><span class="lang {lang_cls}">{esc(lang_lbl)}</span></td>'
            f'<td>{esc(nb.get("creator",""))}</td>'
            f'<td style="white-space:nowrap">{esc(nb.get("modified",""))}</td>'
            f'</tr>'
        )
    nb_rows_html = '\n'.join(nb_rows)
...
<title>Databricks Metadata — {esc(WORKSPACE)}</title>
<style>{CSS}</style>
</head>
<body>
<div class="layout">
...
<div class="main">
  <h1>Databricks Metadata Report</h1>
  ...
  <div class="stats">
    ...
    <div class="sc sc-link" onclick="showTab('notebooks',this)" title="Browse all notebooks with owner and language">
      <div class="sc-n">{n_notebooks}</div><div class="sc-l">Notebooks &#x2192;</div></div>
  </div>
  ...
  <div class="tabs">
    ...
    <div class="tab" onclick="showTab('notebooks',this)" id="tab-notebooks">Notebooks</div>
  </div>
  ...
  <div class="panel" id="p-notebooks">
    <p style="font-size:12px;color:var(--mut);margin-bottom:10px">
      Notebooks discovered via Workspace API. Language and owner come directly from the workspace — not from the Hive Metastore.
    </p>
    <div class="srch">
      <input id="nb-q" placeholder="Search path, language, or owner…" oninput="ft('nb-tbl',this.value)"/>
    </div>
    <div class="tw">
      <table id="nb-tbl">
        <thead><tr><th>Notebook Path</th><th>Language</th><th>Owner</th><th>Last Modified</th></tr></thead>
        <tbody>{nb_rows_html}</tbody>
      </table>
    </div>
  </div>
...
def main():
    ...
    # 3. per-object: columns + DDL + extended info (parallelised)
    ...
    print("\n  Fetching notebooks from workspace API…")
    notebooks = get_notebooks()
    print(f"  Notebooks : {len(notebooks)}")
    from collections import Counter
    for lang, cnt in sorted(Counter(nb.get('language','') or 'unknown' for nb in notebooks).items()):
        print(f"    {lang or 'unknown'}: {cnt}")

    generated = datetime.now().strftime("%Y-%m-%d %H:%M")
    print("\nBuilding HTML…")
    html = build_html(databases, objects, col_data, notebooks, generated)

    out = f"/home/thedavidporter/databricks_metadata_prd_{datetime.now().strftime('%Y%m%d_%H%M')}.html"
